...
def process_and_save_frame(frame_path, frame_anno, ts2idx, segname, output_dir, save_bin):
    ...
    if frame_anno is not None:
        ...
        annos['index'] = np.arange(num_obj, dtype=int32)
        annos['num_points_in_gt'] = frame_anno['num_interior_pts'].to_numpy().astype(int32)
    # frame_info['group_ids'] = np.arange(num_obj, dtype=int32)
