...
class OpenPCDetWaymoDetectionMetricsEstimator(tf.test.TestCase):
    ...
    def generate_waymo_type_results(self, infos, class_names, is_gt=False, fake_gt_infos=True):
        ...
        difficulty = np.concatenate(difficulty).reshape(-1).astype(int8)
    ...
    def build_graph(self, graph):
        with graph.as_default():
            self._pd_frame_id = tf.compat.v1.placeholder(dtype=tf.np.int64)
            ...
            self._gt_frame_id = tf.compat.v1.placeholder(dtype=tf.np.int64)
