    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                if (metric==1) & (m==0) & (l>-1): #2-3d metric=0: bbox, 1: bev, 2: 3d ; m=类别；l=难度 ; k=阈值
                    np.set_printoptions(precision=4, suppress=True)
                    print('mlk:', m, l, k)
                    print('bev_precision02:', precision[m, l, k, :])
                    # print('bev_recall02:', recall[m, l, k, :])
                if (metric==2) & (m==0) & (l>-1): #2-3d metric=0: bbox, 1: bev, 2: 3d ; m=类别；l=难度 ; k=阈值
                    np.set_printoptions(precision=4, suppress=True)
                    print('mlk:', m, l, k)
                    print('3d_precision02:', precision[m, l, k, :])
                    # print('3d_recall02:', recall[m, l, k, :])
